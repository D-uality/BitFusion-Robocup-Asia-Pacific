...
void alignBlack(int alignmentType) {
  if(alignmentType == 0) {
    Serial.println("Aligning to Line from Left");
    runMotors(lineSpeed[1] + 20, lineSpeed[1] + 20, 400);
    runUntil(blackMax, colourPins[0], 0, -lineSpeed[1], lineSpeed[1], 900);
    runUntil(blackWhite, colourPins[4], 0, -lineSpeed[0], lineSpeed[0], 900);
  }

  else if(alignmentType == 1) {
    Serial.println("Aligning to Line from Right");
    runMotors(lineSpeed[1] + 20, lineSpeed[1] + 20, 400);
    runUntil(blackMax, colourPins[3], 0, lineSpeed[1], -lineSpeed[1], 900);
    runUntil(blackWhite, colourPins[1], 0, lineSpeed[0], -lineSpeed[0], 900);
  }

  else if(alignmentType == 2) {
    Serial.println("Aligning to Line from Double");
    runMotors(lineSpeed[1] + 80, -lineSpeed[1] - 80, 1800);
    runUntil(blackMax, colourPins[3], 0, lineSpeed[1], -lineSpeed[1], 900);
    runUntil(blackMax, colourPins[1], 0, lineSpeed[0], -lineSpeed[0], 900);
  }
}

//  ----------------------------------------  Line Following Function
const float outerMultiplier = 5;
const float innerMultiplier = 5;
const float backMultiplier = 2.5;
const int lineSpeed[2] = {230, 320};
// const int forwardSpeed[2] = {800, 890};  //  Ramp Speed Bump speed
int backError = 0;
...
void lineFollow() {
  ...
  runMotors(lineSpeed[0] + lineError, lineSpeed[1] - lineError, 0);
}

//  ----------------------------------------  Green Intersection Functions
const int greenMin[2] = {12, 15};
const int greenMax[2] = {25, 25};
bool leftBlack = 0;
bool rightBlack = 0;
